    ...
    for i, (origin, destination, params) in enumerate(sample_routes):
        ...
        with st.expander(f"Sample {i+1}"):
            ...
            if "error" in prediction:
                ...
                st.write(f"**Price:** {prediction.get('formatted_price', f'₹{prediction.get("price", "N/A")}')}")
                st.write(f"**Distance:** {prediction.get('distance', 'N/A'):.1f} km")
                st.write(f"**Duration:** {prediction.get('duration', 'N/A')} mins")
